...
<head>
    ...
    <title>Edit Company - HSE System</title>
        ...
        <main class="py-8">
            <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6">
                    ...
                    <p class="mt-1 text-sm text-gray-600">Update company information for {{ $company->name }}</p>
                </div>

                <!-- Form -->
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <form method="POST" action="{{ route('companies.update', $company) }}" enctype="multipart/form-data" class="p-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $company->slug) }}" required
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                            <p class="mt-1 text-sm text-gray-500">URL-friendly identifier (e.g., company-name)</p>
                            @error('slug')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $company->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        @if($company->logo_url)
                            <div class="mb-4">
                                ...
                                <div class="mt-1">
                                    <img src="{{ asset('storage/' . $company->logo_url) }}" alt="{{ $company->name }}" class="h-16 w-16 rounded-lg object-cover">
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="flex items-center justify-end">
                            <a href="{{ route('companies.index') }}" class="mr-4 text-gray-600 hover:text-gray-800">Cancel</a>
