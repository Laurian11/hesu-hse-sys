...
<head>
    ...
    <title>{{ $employee->user->name }} - {{ session('current_company')->name ?? 'HSE System' }}</title>
        ...
        <main class="py-8">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">{{ $employee->user->name }}</h2>
                        <p class="mt-1 text-sm text-gray-600">Employee details and information</p>
                    </div>
                    <div class="flex space-x-3">
                        @can('employee.edit')
                            <a href="{{ route('employees.edit', $employee) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
                                Edit Employee
                            </a>
                        @endcan
                        <a href="{{ route('employees.index') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
                            Back to Employees
                        </a>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="p-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="md:col-span-2 flex justify-center">
                                        <img class="h-24 w-24 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($employee->user->name) }}&color=7F9CF5&background=EBF4FF&size=96" alt="{{ $employee->user->name }}">
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">{{ $employee->designation }}</p>
                                    </div>

                                    <!-- Date of Joining -->
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Date of Joining</label>
                                        <p class="mt-1 text-sm text-gray-900">{{ $employee->date_of_joining->format('M d, Y') }}</p>
                                    </div>

                                    <!-- Work Location -->
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Work Location</label>
                                        <p class="mt-1 text-sm text-gray-900">{{ $employee->work_location ?? 'Not specified' }}</p>
                                    </div>

                                    <!-- Status -->
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Status</label>
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $employee->user->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-900">{{ $employee->user->phone ?? 'Not provided' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                <h3 class="text-lg font-medium text-gray-900 mb-4">Employee Stats</h3>

                                <div class="space-y-4">
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm text-gray-600">Days Employed</span>
                                        <span class="text-lg font-semibold text-gray-900">{{ $employee->date_of_joining->diffInDays(now()) }}</span>
                                    </div>

                                    <div class="flex items-center justify-between">
                                        <span class="text-sm text-gray-600">Department</span>
                                        <span class="text-sm font-semibold text-gray-900">{{ $employee->department->code ?? 'N/A' }}</span>
                                    </div>

                                    <div class="flex items-center justify-between">
                                        <span class="text-sm text-gray-600">Role</span>
                                        <span class="text-sm font-semibold text-gray-900">
                                            {{ $employee->user->companies->where('id', session('current_company_id'))->first()?->pivot?->role?->name ?? 'No Role' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-6">
                            <div class="p-6">
                                ...
                                <div class="space-y-3">
                                    @can('user.view')
                                        <a href="{{ route('users.show', $employee->user) }}" class="block w-full bg-gray-100 hover:bg-gray-200 text-gray-900 px-4 py-2 rounded-lg text-sm font-medium text-center">
                                            View User Profile
                                        </a>
                                    @endcan

                                    @can('department.view')
                                        <a href="{{ route('departments.show', $employee->department) }}" class="block w-full bg-blue-100 hover:bg-blue-200 text-blue-900 px-4 py-2 rounded-lg text-sm font-medium text-center">
                                            View Department
                                        </a>
                                    @endcan
                                </div>
                ...
                <div class="mt-6">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            @if($employee->user->activityLogs->count() > 0)
                                <div class="space-y-4">
                                    @foreach($employee->user->activityLogs->take(10) as $log)
